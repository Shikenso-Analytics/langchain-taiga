...
- **`get_entity_by_ref_tool`**: Gets a user story, task or issue by reference. For user stories, the response also includes a `points` field (`{role_name: value}` shape, symmetric to `set_userstory_points_tool`'s input) so a read + write round-trip stays in the same vocabulary. `tags` is returned as flat names (not Taiga's `[name, color]` wire shape) for the same reason — it is exactly what `manage_tags_by_ref_tool` takes as input.
