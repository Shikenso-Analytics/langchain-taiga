...
- **`get_entity_by_ref_tool`**: Gets a user story, task or issue by reference. The response includes `owner` (`{id, username, full_name}` — the creator, which Taiga tracks separately from `assigned_to` and which no longer needs a history walk to recover). For user stories, the response also includes a `points` field (`{role_name: value}` shape, symmetric to `set_userstory_points_tool`'s input) so a read + write round-trip stays in the same vocabulary. `tags` is returned as flat names (not Taiga's `[name, color]` wire shape) for the same reason — it is exactly what `manage_tags_by_ref_tool` takes as input. Pass `include_history=False` when only the current state is needed: history is 84–97% of this payload on real tickets (up to ~190 KB for one busy story, which drops to ~4 KB), and skipping it also saves an API round-trip. The `history` key is then omitted rather than empty, because an empty history is a real answer — Taiga writes no entry for creation, so a never-edited ticket genuinely has none.
