...
- **`get_kanban_board_tool`**: Returns the user-story Kanban board grouped into ordered status columns (mirrors the Taiga UI). Each column carries `status`, `status_id`, `order`, `is_closed`, `wip_limit` and `cards` (sorted by `kanban_order`); `include_closed=False` hides closed columns and, since 2.19.0, asks Taiga for open stories only instead of listing the whole project first. `statuses` (exact names or ids) limits the board to those columns and is pushed down the same way. With `fields`, cards can also carry `modified_date`, `created_date`, `due_date`, `tags` and `is_blocked`, and since 2.20.0 `tasks`: a summary of every task of the story (`id`, `ref`, `subject`, `status_id`, `is_closed`, `is_blocked`, `is_iocaine`), fetched in the same request via Taiga's `include_tasks=1`. Three more card keys cost one request per card and come only when a path names them: `custom_attributes` (values by attribute id), and `last_activity_at` / `last_comment_at` for the users in `activity_users` (usernames, ids or `me`). The server reads them in parallel, and a card that still fails after one retry fails the whole call rather than coming back empty. User stories only.
- **`get_entity_by_ref_tool`**: Gets a user story, task or issue by reference. The response includes `owner` (`{id, username, full_name}` — the creator, which Taiga tracks separately from `assigned_to` and which no longer needs a history walk to recover). For user stories, the response also includes a `points` field (`{role_name: value}` shape, symmetric to `set_userstory_points_tool`'s input) so a read + write round-trip stays in the same vocabulary. `tags` is returned as flat names (not Taiga's `[name, color]` wire shape) for the same reason — it is exactly what `manage_tags_by_ref_tool` takes as input. Pass `include_history=False` when only the current state is needed: history is 84–97% of this payload on real tickets (up to ~190 KB for one busy story, which drops to ~4 KB), and skipping it also saves an API round-trip. The `history` key is then omitted rather than empty, because an empty history is a real answer — Taiga writes no entry for creation, so a never-edited ticket genuinely has none. Since 2.19.0, `fields` fetches only what the listed paths need (history, tasks, custom attributes and user lookups are skipped otherwise), and `history_since` / `history_user` (`me`, a username or an id) / `history_comments_only` / `history_limit` filter the history rows, with `history_total` and `history_returned` saying how much was left out. Since 2.22.0 `related.tasks.history` embeds each task's history, filtered like the story's; it needs `history_since` and reads only the tasks modified since then (one call for a 306-task story: 63 KB, 3.9 s). Since 2.20.0 a `fields` path can also ask for `status_id`, a user story's `assigned_users` (`{id, username, full_name}` each) and `related.tasks.id` / `status_id` / `modified_date`; none of them appear in the default answer.
- **`update_entities_by_ref_tool`** (2.22.0): Applies one change (status, assignee, watchers, tags, comment) to up to 100 entities of one type in a single call — each gets exactly what `update_entity_by_ref_tool(strict=True)` would do for it: its own PATCH, its own `expected_version` and its own answer (with `target`, and `state` when `read_back` is on, the default). An item can bring its own comment. One entity failing does not stop the others; `succeeded` and `failed` list the refs. Built for hand-overs of many tasks at once.
- **`update_entity_by_ref_tool`**: Updates a user story, task or issue by reference. Since 2.18.0 this includes `milestone` — the sprint name, or `current` for the sprint covering today, or an empty string to take it out of its sprint. Sprint membership previously had no MCP path at all and needed a hand-rolled REST `PATCH`. User stories and issues only: a task's sprint follows its user story, and epics have none. Since 2.19.0 it also takes `comment`, `watchers` (+ `watchers_mode`) and `tags` (+ `tags_mode`) and sends everything in **one** PATCH, so a hand-over leaves one history entry. `strict=True` resolves status and assignee only exactly (never through the language model, which otherwise matches "finished" to "Done"), and `read_back=True` returns what Taiga stored (`state`, the newest `history_entry`, and `comment_entries`). Since 2.20.0 `assigned_users` sets a user story's complete assignee list in the same PATCH, always resolved exactly, and `read_back` reports it; `state.ids` carries the stored status, assignee, watchers and assignees as ids, which is what a scripted write compares against. Since 2.21.0 `expected_version` ties a write to the version the caller read (`get_entity_by_ref_tool` returns it as `version` on request): if the entity has changed since, nothing is written and the answer is a 409 with `current_version`. A new-style answer also names its `target` (project, ref, type).
...
- **`get_custom_attributes_tool`**: Reads custom-attribute values from a user story, task or issue. Takes `fields` and `compact`.
...
- **`list_project_members_tool`**: Lists all members of a project with their roles (email opt-in). Takes `fields` and `compact`.

### Token-lean reads

Every byte a tool returns lands in the calling model's context. The read tools above take two
parameters (2.19.0) that keep that small:

- **`fields`**: a list of dotted paths, e.g. `["subject", "related.tasks.ref", "history.comment"]`.
  Lists are walked element by element. A path whose first part is not a key of the answer fails
  the call and names the valid keys. A requested key that an element lacks comes back as `null`.
  `get_entity_by_ref_tool` and `get_kanban_board_tool` fetch only what the paths need, so this
  saves Taiga requests too. With `fields`, the answer echoes the request as `query`.
...
| Call | Answer | Time |
|---|---|---|
| One user story, default | 85.9 KB | 4.1 s |
| The same story, `fields` + `compact`, no history | 1.7 KB | 1.0 s |
